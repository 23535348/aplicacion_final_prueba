...
var categoriaM = require('./service/_models/categorias.model');
var datos_login ="";
...
// Autenticar usuarios
    ...
    app.get('/categorias', function(req, res) {
        categoriaM.find({}, function(err, docs) {
            if(err) return console.error(err);
            res.json(docs);
        });
    });
    ...
    // count all
    app.get('/categorias/count', function(req, res) {
        categoriaM.count(function(err, count) {
            if(err) return console.error(err);
            res.json(count);
        });
    });
    ...
    app.post('/categoria', function(req, res) {
        var obj = new categoriaM({ nombre : req.body.categoria});
        obj.save(function(err, obj) {
            if(err) return console.error(err);
            res.status(200).json(obj);
        });
